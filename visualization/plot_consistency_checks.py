...
def main(
    ...
    if separate_orders is True:
            ...
            for j in range(len(orders)-h):
                ...
                for i, p_decimal in enumerate(p_values):
                    succ_perc, N = find_percent_success(
                        ...
                        orders[j:j+h+1], Lambda_b, lambda_mult, X_ref_hash, p_decimal,
                        prior_set, h, convention,
                        cbar_lower, cbar_upper, sigma,
                        potential_info=None)
                    consistency_list.append(100*succ_perc)
                plot_label = orders[j]  # + r" $\lambda =" + str(lambda_mult) + r"$"
        ...
        for j, lambda_mult in enumerate(lambda_mult_list):
            ...
            for i, p_decimal in enumerate(p_values):
                succ_perc, N = find_percent_success(
                    error_band_dir, observable_list, theta_values, energy_values,
                    orders, Lambda_b, lambda_mult, X_ref_hash, p_decimal,
                    prior_set, h, convention,
                    cbar_lower, cbar_upper, sigma,
                    potential_info=None)
                consistency_list.append(100*succ_perc)
            ax.plot(p_percent_list, consistency_list, label=r"$\lambda =" + str(lambda_mult) + r"$", marker=marker_list[j])
    ...
    ax.plot(binom_95_u, binom_CI_domain, color="black", ls="--")
    ax.plot(binom_95_l, binom_CI_domain, color="black", ls="--")
    ax.fill_betweenx(
    ...
    ax.plot(binom_68_u, binom_CI_domain, color="black", ls="--")
    ax.plot(binom_68_l, binom_CI_domain, color="black", ls="--")
    ax.fill_betweenx(
    ...
    plot_name = plot_consistency_filename(
        ...
        orders, Lambda_b, lambda_mult_list,
