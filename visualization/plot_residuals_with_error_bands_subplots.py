...
def main(
    ...
    if indep_var == "theta":
        ...
        param_var_label = r"$E_{\text{lab}}"
        param_var_units = r"$\,MeV"
    else:
        param_var = "theta"
        indep_var_label = r"$E_{\text{lab}}$ (MeV)"
    ...
    for observable in observable_list:
        ...
        for param in param_list:
            ...
            #         if i != 4:
            #             axis.get_xaxis().set_ticklabels(["", "", 60, 120, ""])
            ...
            ymin = res_min - .1 * abs(res_min)
            ymax = res_max + .1 * abs(res_max)
            ...
            for i, order in enumerate(orders):
                ...
                # print(npwa_data[48], npwa_data[49])
                res = (array(dob_data) - npwa_data)
                for sub_plt in range(i, len(orders)):
                    # if len(orders) == 4:
                    #     sub_plt = sub_plt_index + 1
                    # zorder must be <= 2 or else it will cover axes and ticks
                    ax[sub_plt].plot(x, res, color=color_dict[order](.99),
                                     zorder=i/5)

                    # ax[sub_plt].plot(x, np.zeros(len(x)), zorder=10,
                    #                  ls="-.", color="black")
                ...
                # Plot the error bands
                    for band_num, p in enumerate(sorted(p_decimal_list,
                        ...
                        # print(array(dob_data_lower)[48], array(dob_data_lower)[49])
                        res_lower = (array(dob_data_lower) - npwa_data)
                        res_upper = (array(dob_data_upper) - npwa_data)
                        # print(array(res_lower)[48], array(res_lower)[49])
            ...
            # elif len(orders) == 4:
            #     ax[0].text(
            #         0.05, 0.95, text_str, transform=ax[0].transAxes,
            #         fontsize=10, verticalalignment='top')
                # leg = ax[0].legend([extra], [text_str], loc="best", handlelength=0, handletextpad=0, fancybox=True, prop={'size': 10})
            ...
            #             loc=8, ncol=2, borderaxespad=0.,  # mode="expand",
            ...
            # leg = leg_ax.legend([extra], [text_str], loc="best", handlelength=0, handletextpad=0, fancybox=True, prop={'size': 8})
            ...
            plot_name = subplot_res_error_bands_filename(
